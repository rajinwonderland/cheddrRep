...
import {Card, CardItem, Thumbnail, ListItem} from 'native-base'
...
const GooglePlacesAutocomplete = React.createClass({
  ...
  _getRowLoader() {
    if (Platform.OS === 'android') {
      return (
        <ProgressBarAndroid
          style={defaultStyles.androidLoader}
          styleAttr="Inverse"
        />
      );
    }
  ...
  _getListView() {
    ...
    if(this.props.enablePoweredByContainer) {
      return (
        <View
          style={defaultStyles.poweredContainer}
        >
          <Text>Powered by Novvum</Text>
        </View>
      );
    }
